...
@RequestMapping("/user")//映射到类上 localhost：8080/user
...
// @RequestMapping(method = RequestMethod.POST)。
     ...
     public ResponseEntity<?> createUser(@RequestBody User user) {
        if(user.getCpNameSei()== null || user.getCpNameSei().isEmpty()){
         ...
         if(user.getCpNameSeiKana()== null || user.getCpNameMei().isEmpty()){
         ...
         if(user.getCpOtherCountry()== null || user.getCpOtherCountry().isEmpty()){
             return ResponseEntity.badRequest().body("姓（ローマ字）を入力してください。");
         }
         ...
         if(user.getCpShokugyoCode()== null || user.getCpShokugyoCode().isEmpty()){
             return ResponseEntity.badRequest().body("姓（ローマ字）を入力してください。");
         }
         if(user.getCpKinmusakiName()== null || user.getCpKinmusakiName().isEmpty()){
             return ResponseEntity.badRequest().body("姓（ローマ字）を入力してください。");
         }
